...
export default function MutakhafyGameOver({ data, token, onFinish }) {
  ...
  return (
    <div style={{ paddingTop: 16, animation: "fadeIn 0.5s ease" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 14, color: C.muted, marginTop: 4 }}>المتخفي</div>
      </div>
        ...
        {[1, 0, 2].map((idx) => {
          ...
          return (
            <div key={p.token} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, animation: `su 0.5s ${idx * 0.15}s backwards`, flex: "1 1 0", maxWidth: 120 }}>
              ...
              <span style={{ fontSize: "min(12px, 3vw)", fontWeight: 700, color: p.token === token ? pink : "#fff", maxWidth: "100%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</span>
              <div style={{
      ...
      {myRank && (
        <div style={{
          ...
          background: `${isChamp ? C.gold : pink}08`, border: `1px solid ${isChamp ? C.gold : pink}20`,
          ...
          <div style={{ width: 1, background: "rgba(255,255,255,0.05)" }} />
          <div>
            <div style={{ fontSize: 26, fontWeight: 900, color: myRank.wasEverGuessed ? C.orange : C.gold }}>
              {myRank.wasEverGuessed ? "🔍" : "👻"}
            </div>
            <div style={{ fontSize: 10, color: C.muted }}>
              {myRank.wasEverGuessed ? "انكشفت" : "شبح كامل!"}
            </div>
          </div>
        </div>
      )}
      ...
      {myRank && (
        <div style={{
          ...
          background: `${pink}08`, border: `1px solid ${pink}20`, borderRadius: 12,
        }}>
          <div style={{ fontSize: 11, color: C.muted, marginBottom: 4 }}>كنت متخفي بإسم</div>
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 24 }}>{myRank.fakeAvatar}</span>
            <span style={{ fontSize: 16, fontWeight: 800, color: pink }}>{myRank.fakeName}</span>
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        {rankings.map((p) => (
          <div key={p.token} style={{
            ...
            <span style={{ flex: 1, fontSize: 14, fontWeight: 700, color: p.token === token ? pink : "#fff" }}>
              {p.name} {p.token === token && "(أنت)"}
            </span>
            <span style={{ fontSize: 11, color: C.muted }}>{p.fakeAvatar} {p.fakeName}</span>
            <span style={{ fontSize: 13, fontWeight: 800, color: C.gold }}>{p.score} ⭐</span>
          </div>
        ))}
      </div>

      <Btn color={pink} onClick={() => onFinish(rankings)}>🔄 مرة ثانية</Btn>
    </div>
  );
}
